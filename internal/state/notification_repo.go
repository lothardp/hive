...
func (r *NotificationRepository) Create(ctx context.Context, n *Notification) error {
	...
	result, err := r.db.ExecContext(ctx,
		`INSERT INTO notifications (cell_name, title, message, details, created_at)
		 VALUES (?, ?, ?, ?, ?)`,
		n.CellName, n.Title, n.Message, n.Details, now,
	)
	if err != nil {
...
func (r *NotificationRepository) List(ctx context.Context, cellName string, unreadOnly bool) ([]Notification, error) {
	query := `SELECT id, cell_name, title, message, details, read, created_at FROM notifications WHERE 1=1`
	var args []any
	...
	for rows.Next() {
		var n Notification
		if err := rows.Scan(&n.ID, &n.CellName, &n.Title, &n.Message, &n.Details, &n.Read, &n.CreatedAt); err != nil {
...
func (r *NotificationRepository) GetByID(ctx context.Context, id int64) (*Notification, error) {
	...
	var n Notification
	err := row.Scan(&n.ID, &n.CellName, &n.Title, &n.Message, &n.Details, &n.Read, &n.CreatedAt)
